/* 
 ******************************************************************************
 * File: OGlycanSpecSpliter.java * * * Created on 2012-12-14
 *
 ...
 * @author ck
 *
 * @version 2012-12-14, 14:00:39
 */
public class OGlycanSpecSpliter {
	...
	private OGlycanUnit[] simpleUnits = new OGlycanUnit[] { OGlycanUnit.core1_1, OGlycanUnit.core1_2,
			OGlycanUnit.core1_3, OGlycanUnit.core1_4, OGlycanUnit.core1_4b, OGlycanUnit.core1_5, OGlycanUnit.core1_5b,
	...
	public OGlycanSpecSpliter(String file) throws DtaFileParsingException, FileNotFoundException {
		this(new File(file));
	}

	public OGlycanSpecSpliter(File file) throws DtaFileParsingException, FileNotFoundException {
	...
	private void judge(MS2Scan ms2scan) {
		...
		L: for (int id = 0; id < noglyPeaksIntenSort.length; id++) {
			...
			if (mzi + 203.079373 > mzmax) {
				...
				L1: for (int j = i + 1; j < noglyPeaks.length; j++) {
					...
					if (tolei < tolerance) {
						...
						for (int l = 0; l < this.typesets.length; l++) {
							...
							if (typesets[l].contains(unitname)) {
								...
								String info = (l + 1) + "\t1\t" + units[j].getName() + "\t" + units[j].getMass() + "\t"
										+ pepmz + "\t" + name + "\t" + monoinfo + lineSeparator;
				...
				for (int j1 = 0; j1 < units.length; j1++) {
					for (int j2 = 0; j2 < units.length; j2++) {
						...
						if (tolei < tolerance) {
							...
							for (int k = i + 1; k < noglyPeaks.length; k++) {
								tolei = Math
										.abs(mw - noglyPeaks[k].getMz() - units[j1].getMass() - units[j2].getMass());
								if (tolei < tolerance) {
									if (noglyPeaks[k].getIntensity() > pepinten) {
										pepmz = noglyPeaks[k].getMz();
										pepinten = noglyPeaks[k].getIntensity();
							...
							for (int l = 0; l < this.typesets.length; l++) {
								if (typesets[l].contains(unitname1) && typesets[l].contains(unitname2)) {
									...
									String info = (l + 1) + "\t2\t" + units[j1].getName() + "\t" + units[j1].getMass()
											+ "\t" + units[j2].getName() + "\t" + units[j2].getMass() + "\t" + pepmz
				...
				for (int j1 = 0; j1 < units.length; j1++) {
					for (int j2 = 0; j2 < units.length; j2++) {
						for (int j3 = 0; j3 < units.length; j3++) {
							...
							double tolei = Math
									.abs(mw - mzi - units[j1].getMass() - units[j2].getMass() - units[j3].getMass());
							...
							if (tolei < tolerance) {
								...
								for (int k = i + 1; k < noglyPeaks.length; k++) {
									tolei = Math.abs(mw - noglyPeaks[k].getMz() - units[j1].getMass()
								...
								for (int l = 0; l < this.typesets.length; l++) {
									if (typesets[l].contains(unitname1) && typesets[l].contains(unitname2)
										...
										String info = (l + 1) + "\t3\t" + units[j1].getName() + "\t"
												...
												+ units[j3].getMass() + "\t" + pepmz + "\t" + name + "\t" + monoinfo
												+ lineSeparator;
	 ...
	 * 
	 * IMS2PeakList peaklist = ms2scan.getPeakList(); PrecursePeak pp =
	 * peaklist.getPrecursePeak(); double mh = pp.getMH(); double mz =
	 * pp.getMz(); short charge = pp.getCharge(); String name =
	 * ms2scan.getScanName().getScanName(); if(name.endsWith(", ")) name =
	 * name.substring(0, name.length()-2); double intenthres1 = 0; double
	 * intenthres2 = peaklist.getBasePeak().getIntensity()*0.05;
	 * 
	 * IPeak [] temppeaks = peaklist.getPeaksSortByIntensity();
	 * if(temppeaks.length>=300){ intenthres1 = 0.4; }else
	 * if(temppeaks.length<300 && temppeaks.length>=200){ intenthres1 = 0.35;
	 * }else if(temppeaks.length<200 && temppeaks.length>=140){ intenthres1 =
	 * 0.28; }else if(temppeaks.length<140 && temppeaks.length>=70){ intenthres1
	 * = 0.2; }else{ intenthres1 = 0.1; }
	 * 
	 * ArrayList <Double> tempintenlist = new ArrayList <Double>(); double []
	 * rsdlist = new double[temppeaks.length]; double percent = 0; for(int
	 * i=temppeaks.length-1;i>=0;i--){
	 * tempintenlist.add(temppeaks[i].getIntensity());
	 * rsdlist[tempintenlist.size()-1] = MathTool.getRSD(tempintenlist); }
	 * 
	 * for(int i=1;i<=rsdlist.length;i++){
	 * 
	 * if(rsdlist[rsdlist.length-i]<intenthres1){ intenthres2 =
	 * temppeaks[i-1].getIntensity(); percent =
	 * ((double)temppeaks.length-i+1)/((double)temppeaks.length); break; } }
	 * 
	 * IPeak [] peaks = peaklist.getPeakList(); Arrays.sort(peaks);
	 * 
	 * double maxinten = 0; int [] markpeaks = new int [7];
	 * 
	 * ArrayList <IPeak> lowIntenList = new ArrayList <IPeak>(); ArrayList
	 * <IPeak> highIntenList = new ArrayList <IPeak>();
	 * 
	 * StringBuilder sb = new StringBuilder();
	 * sb.append("BEGIN IONS"+lineSeparator);
	 * sb.append("PEPMASS="+mz+lineSeparator);
	 ...
	 * for(int i=0;i<peaks.length;i++){
	 * 
	 * double mzi = peaks[i].getMz(); double inteni = peaks[i].getIntensity();
	 * 
	 * if(inteni<intenthres2){ lowIntenList.add(peaks[i]); }else{
	 * if(Math.abs(mzi-163.060101)<tolerance){ continue; }else
	 * if(Math.abs(mzi-204.086649)<tolerance){ continue; }else
	 * if(Math.abs(mzi-274.087412635)<tolerance){ markpeaks[0] = 1; continue;
	 * }else if(Math.abs(mzi-292.102692635)<tolerance){ markpeaks[0] = 1;
	 * continue; }else if(Math.abs(mzi-366.139472)<tolerance){ markpeaks[1] = 1;
	 * continue; }else if(Math.abs(mzi-495.18269)<tolerance){ markpeaks[2] = 1;
	 * continue; }else if(Math.abs(mzi-583.19873)<tolerance){ markpeaks[3] = 1;
	 * continue; }else if(Math.abs(mzi-407.16665)<tolerance){ markpeaks[4] = 1;
	 * continue; }else if(Math.abs(mzi-569.218847)<tolerance){ markpeaks[5] = 1;
	 * continue; }else if(Math.abs(mzi-731.271672)<tolerance){ markpeaks[6] = 1;
	 * continue; }else if(Math.abs(mzi-528.192299)<tolerance){
	 * if(inteni>intenthres2) return; }else
	 * if(Math.abs(mzi-690.245124)<tolerance){ if(inteni>intenthres2) return;
	 * }else{ highIntenList.add(peaks[i]); if(peaks[i].getIntensity()>maxinten){
	 * maxinten = peaks[i].getIntensity(); } continue; } } }
	 * 
	 * if(highIntenList.size()+lowIntenList.size()==peaks.length ||
	 * highIntenList.size()<=5) return;
	 * 
	 * glycocount++; IPeak [] noglyPeaks = highIntenList.toArray(new
	 * IPeak[highIntenList.size()]); double mzmax =
	 * noglyPeaks[noglyPeaks.length-1].getMz(); double pepmz = 0; double
	 * pepinten = 0;
	 * 
	 * HashSet <Double> frags = new HashSet <Double>(); String info = "";
	 * StringBuilder monoinfo = new StringBuilder(); for(int
	 * i=0;i<markpeaks.length;i++){ monoinfo.append(markpeaks[i]).append("_"); }
	 * int type = -1;
	 * 
	 * L: for(int i=0;i<noglyPeaks.length;i++){
	 * 
	 * double mzi = noglyPeaks[i].getMz(); double inteni =
	 * noglyPeaks[i].getIntensity();
	 * 
	 * boolean pass = false; if(mzi+203.079373>mzmax){ pass = true; }else{
	 * for(int j=i+1;j<noglyPeaks.length;j++){ double mzj =
	 * noglyPeaks[j].getMz(); if(Math.abs(mzj-mzi-203.079373)<tolerance){ pass =
	 * true; break; }else if(Math.abs(mzj-mzi-203.079373+18.010565)<tolerance){
	 * pass = true; break; } } } if(!pass) continue;
	 * System.out.println(pass+"\t"+mzi+"\t"); for(int j=0;j<units.length;j++){
	 * 
	 * double tolei = Math.abs(mh-mzi-units[j].getMass());
	 * System.out.println("1\t"+pass+"\t"+mzi+"\t"+tolei+"\t"+mh+"\t"+units[j].
	 * getMass()); if(tolei<tolerance){
	 * 
	 * pepmz = mzi; pepinten = inteni; String unitname = units[j].getName();
	 * 
	 * for(int k=i+1;k<noglyPeaks.length;k++){ tolei =
	 * Math.abs(mh-noglyPeaks[k].getMz()-units[j].getMass());
	 * if(tolei<tolerance){ if(noglyPeaks[k].getIntensity()>pepinten){ pepmz =
	 * noglyPeaks[k].getMz(); pepinten = noglyPeaks[k].getIntensity(); } }else{
	 * for(int l=0;l<this.typesets.length;l++){
	 * if(typesets[l].contains(unitname)){
	 * 
	 * if(l==2 && markpeaks[0]==1){ pepinten = 0; // continue L; }
	 * if(pepinten/maxinten<0.4){ pepinten = 0; // continue L; }
	 * 
	 * info = (l+1)+"\t1\t"+units[j].getName()+"\t"+
	 * units[j].getMass()+"\t"+pepmz+"\t"+name+"\t"+monoinfo+lineSeparator; type
	 * = l;
	 * 
	 * double [] fraglist = units[j].getFragment(); for(int
	 * m=0;m<fraglist.length;m++){ frags.add(fraglist[m]); }
	 * 
	 * break L; } } } } } }
	 * 
	 * for(int j1=0;j1<units.length;j1++){ for(int j2=0;j2<units.length;j2++){
	 * 
	 * double tolei = Math.abs(mh-mzi-units[j1].getMass()-units[j2].getMass());
	 * System.out.println("2\t"+pass+"\t"+mzi+"\t"+tolei+"\t"+mh+"\t"+units[j1].
	 * getMass()+"\t"+units[j2].getMass()); if(tolei<tolerance){
	 * 
	 * pepmz = mzi; pepinten = inteni;
	 * 
	 * String unitname1 = units[j1].getName(); String unitname2 =
	 * units[j2].getName();
	 * 
	 * for(int k=i+1;k<noglyPeaks.length;k++){ tolei =
	 * Math.abs(mh-noglyPeaks[k].getMz()-units[j1].getMass()-units[j2].getMass()
	 * ); if(tolei<tolerance){ if(noglyPeaks[k].getIntensity()>pepinten){ pepmz
	 * = noglyPeaks[k].getMz(); pepinten = noglyPeaks[k].getIntensity(); }
	 * }else{ for(int l=0;l<this.typesets.length;l++){
	 * if(typesets[l].contains(unitname1) && typesets[l].contains(unitname2)){
	 * 
	 * if(l==2 && markpeaks[0]==1){ pepinten = 0; // continue L; }
	 * if(pepinten/maxinten<0.4){ pepinten = 0; // continue L; }
	 ...
	 * type = l;
	 * 
	 * double [] fraglist1 = units[j1].getFragment(); for(int
	 * m=0;m<fraglist1.length;m++){ frags.add(fraglist1[m]); }
	 * 
	 * double [] fraglist2 = units[j2].getFragment(); for(int
	 * m=0;m<fraglist2.length;m++){ frags.add(fraglist2[m]); }
	 * 
	 * break L; } } } } } } }
	 * 
	 * for(int j1=0;j1<units.length;j1++){ for(int j2=0;j2<units.length;j2++){
	 ...
	 * 
	 * double tolei =
	 * Math.abs(mh-mzi-units[j1].getMass()-units[j2].getMass()-units[j3].getMass
	 * ());
	 * System.out.println("3\t"+pass+"\t"+mzi+"\t"+tolei+"\t"+mh+"\t"+units[j1].
	 * getMass()+"\t"+units[j2].getMass()+"\t"+units[j3].getMass());
	 * 
	 * if(tolei<tolerance){
	 * 
	 * pepmz = mzi; pepinten = inteni;
	 * 
	 * String unitname1 = units[j1].getName(); String unitname2 =
	 * units[j2].getName(); String unitname3 = units[j3].getName();
	 * 
	 * for(int k=i+1;k<noglyPeaks.length;k++){ tolei =
	 * Math.abs(mh-noglyPeaks[k].getMz()-units[j1].getMass()-units[j2].getMass()
	 * -units[j3].getMass()); if(tolei<tolerance){
	 * if(noglyPeaks[k].getIntensity()>pepinten){ pepmz = noglyPeaks[k].getMz();
	 * pepinten = noglyPeaks[k].getIntensity(); } }else{ for(int
	 * l=0;l<this.typesets.length;l++){ if(typesets[l].contains(unitname1) &&
	 * typesets[l].contains(unitname2) && typesets[l].contains(unitname3)){
	 * 
	 * if(l==2 && markpeaks[0]==1){ pepinten = 0; // continue L; }
	 * if(pepinten/maxinten<0.4){ pepinten = 0; // continue L; }
	 * 
	 ...
	 * m=0;m<fraglist2.length;m++){ frags.add(fraglist2[m]); }
	 * 
	 * double [] fraglist3 = units[j3].getFragment(); for(int
	 * m=0;m<fraglist3.length;m++){ frags.add(fraglist3[m]); }
	 * 
	 * break L; } } } } } } } } } //System.out.println(pepmz+"\t"+type);
	 * if(pepmz==0 || type==-1){ this.dislist.add(ms2scan); return; }
	 * 
	 * int peakcount = 0; Arrays.sort(noglyPeaks); for(int
	 * i=0;i<noglyPeaks.length;i++){
	 * 
	 * if(Math.abs(pepmz-noglyPeaks[i].getMz())<tolerance){ continue; }
	 * 
	 * boolean use = true; Double [] fraglist = frags.toArray(new
	 * Double[frags.size()]); Arrays.sort(fraglist); for(int
	 * j=0;j<fraglist.length;j++){
	 * if(Math.abs(pepmz+fraglist[j]-noglyPeaks[i].getMz())<=tolerance ||
	 * Math.abs(pepmz+fraglist[j]-18.010565-noglyPeaks[i].getMz())<=tolerance){
	 * 
	 * use = false;
	 * 
	 * break; } } if(use){ peakcount++;
	 * sb.append(noglyPeaks[i].getMz()+"\t"+noglyPeaks[i].getIntensity()+
	 * lineSeparator); } }
	 * 
	 * if(peakcount<=5) return;
	 * 
	 * pw.write(info); sb.append("END IONS"+lineSeparator);
	 * this.splist[type].add(sb.toString()); }
	 * 
	 * public void writeDis(String out) throws IOException{ PrintWriter pw = new
	 * PrintWriter(out); for(int i=0;i<this.dislist.size();i++){
	 * 
	 * MS2Scan ms2scan = dislist.get(i); IMS2PeakList peaklist =
	 * ms2scan.getPeakList(); PrecursePeak pp = peaklist.getPrecursePeak();
	 * double mz = pp.getMz(); short charge = pp.getCharge(); String name =
	 * ms2scan.getScanName().getScanName(); if(name.endsWith(", ")) name =
	 ...
	 * sb.append("PEPMASS="+mz+lineSeparator);
	 * sb.append("CHARGE="+charge+"+"+lineSeparator);
	 * sb.append("TITLE="+name+lineSeparator); for(int j=0;j<peaks.length;j++){
	 * sb.append(peaks[j].getMz()+"\t"+peaks[j].getIntensity()+lineSeparator); }
	 * sb.append("END IONS"+lineSeparator); pw.write(sb.toString()); }
	 * pw.close(); }
	 */
	...
	public static void main(String[] args) throws DtaFileParsingException, IOException {
		String file = "H:\\OGlycan\\��׼\\20120323_kappa-casein_after-enrichment_5ug-01.mgf";
		// String temp =
		// "H:\\OGlycan\\Ѫ��\\20120328-humanserum-trypsin\\20120328_humaneserum_trypsin_HILIC_8uL-03.mgf";
