/* 
 ******************************************************************************
 * File: OGlycanSpec2DSpliter.java * * * Created on 2013-6-21
 *
 ...
 * @author ck
 * 
 * @version 2013-6-21, 19:28:34
 */
public class OGlycanSpec2DSpliter {
...
//	private MgfReader reader;
	...
	private OGlycanUnit[] simpleUnits = new OGlycanUnit[] {
			...
			OGlycanUnit.core1_4, OGlycanUnit.core1_4b, OGlycanUnit.core1_5,
			OGlycanUnit.core1_5b, OGlycanUnit.core2_1, OGlycanUnit.core2_2,
	...
	public OGlycanSpec2DSpliter(String file) throws IOException {
		this(new File(file));
	}

	public OGlycanSpec2DSpliter(File file) throws IOException {

		this.file = file;
		String out = file.getAbsolutePath() + "\\peps.info";
	...
	public void split() throws DtaFileParsingException, IOException {
		
		File [] mgfs = this.file.listFiles(new FileFilter(){
		...
		for(int i=0;i<mgfs.length;i++){
			...
			System.out.println(i+1+"\t"+mgfs[i].getName());
			
			MgfReader reader = new MgfReader(mgfs[i]);
	...
	private void judge(MS2Scan ms2scan, int fileid) {
		...
		name = name.replace("1.1.1", "1.1."+fileid);
		...
		L: for (int id = 0; id < noglyPeaksIntenSort.length; id++) {
			...
			if (mzi + 203.079373 > mzmax) {
				...
				L1: for (int j = i + 1; j < noglyPeaks.length; j++) {
					...
					for (int chargeid = 1; chargeid <= charge; chargeid++) {
						if (Math.abs((mzj - mzi) * (double) chargeid
								- 203.079373) < tolerance) {
			...
			for (int j = 0; j < units.length; j++) {
				...
				if (tolei < tolerance) {
					...
					for (int k = i + 1; k < noglyPeaks.length; k++) {
						tolei = Math.abs(mw - noglyPeaks[k].getMz()
								- units[j].getMass());
						if (tolei < tolerance) {
			...
			for (int j1 = 0; j1 < units.length; j1++) {
				for (int j2 = 0; j2 < units.length; j2++) {
					...
					double tolei = Math.abs(mw - mzi - units[j1].getMass()
							- units[j2].getMass());
					if (tolei < tolerance) {
						...
						for (int l = 0; l < this.typesets.length; l++) {
							if (typesets[l].contains(unitname1)
									&& typesets[l].contains(unitname2)) {

								String info = (l + 1) + "\t2\t"
										+ units[j1].getName() + "\t"
										+ units[j1].getMass() + "\t"
										+ units[j2].getName() + "\t"
										+ units[j2].getMass() + "\t" + pepmz
										+ "\t" + name + "\t" + monoinfo
										+ lineSeparator;
			...
			for (int infoid = 0; infoid < infolist.size(); infoid++) {
				...
				for (int i = 0; i < noglyPeaks.length; i++) {
					...
					for (int j = 0; j < fraglist.length; j++) {
						if (Math.abs(pepmz + fraglist[j]
								- noglyPeaks[i].getMz()) <= tolerance
								|| Math.abs(pepmz + fraglist[j] - 18.010565
										- noglyPeaks[i].getMz()) <= tolerance) {
