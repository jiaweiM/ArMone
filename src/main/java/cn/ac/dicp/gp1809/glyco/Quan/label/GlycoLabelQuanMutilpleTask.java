/* 
 ******************************************************************************
 * File: GlycoLabelQuanMutilpleTask.java * * * Created on 2014��6��6��
 *
 ...
 * @author ck
 *
 * @version 2014��6��6��, ����10:22:01
 */
public class GlycoLabelQuanMutilpleTask implements ITask {

	private IPeptideListReader [] readers;
	private IPeptideListReader currentReader;
	 ...
	 */
	public GlycoLabelQuanMutilpleTask(IPeptideListReader[] readers, String peakfile, String result, 
		...
		this.currentReaderId = 0;
		this.glycoType = glycanType;
		this.type = labeltype;
		
		this.idenLabel = jpara.getDeGlycoLabel();
		this.aam = currentReader.getSearchParameter().getVariableInfo();
...
//				this.getter = new OGlyLabelGetter(peakfile, jpara, labeltype, symset);
		...
		ProteinNameAccesser accesser = currentReader.getProNameAccesser();
		for(int i=1;i<readers.length;i++){
			accesser.appand(readers[i].getProNameAccesser());
		}
	 ...
	 * @throws IOException
	 */
	public GlycoLabelQuanMutilpleTask(IPeptideListReader[] readers, NGlyStrucLabelGetter getter, 
	...
	public boolean hasNext() {
		...
		try {
			...
			boolean has = true;
			this.curtPeptide = this.currentReader.getPeptide();
			if(this.curtPeptide==null){
				if(this.currentReaderId==this.readers.length-1){
					has = false;
				}else{
					this.currentReader = this.readers[this.currentReaderId++];
					this.curtPeptide = this.currentReader.getPeptide();
					if(this.curtPeptide==null){
						has = false;
					}
	...
	private static void test() throws FileDamageException, IOException, FastaDataBaseException, 
		...
		String[] ppls = new String[]{};
		String peak = "H:\\NGLYCO_QUAN\\NGlycan_Quan_20130812\\4Glyco_protein\\Glycan\\20130805_4p_di-labeling_HCD_N-glycan_quantification_1_1-2.mzXML";
		String pxml = "H:\\NGLYCO_QUAN\\NGlycan_Quan_20130812\\4Glyco_protein\\Glycan\\20130805_4p_di-labeling_HCD_N-glycan_quantification_1_1-2.pxml";
		String out = "H:\\NGLYCO_QUAN\\NGlycan_Quan_20130812\\4Glyco_protein\\Glycan\\20130805_4p_di-labeling_HCD_N-glycan_quantification_1_1-2.xls";
		...
		IPeptideListReader[] readers = new PeptideListReader[ppls.length];
		for(int i=0;i<ppls.length;i++){
			readers[i] = new PeptideListReader(ppls[i]);
		}

		GlycoLabelQuanMutilpleTask task = new GlycoLabelQuanMutilpleTask(readers, peak, pxml, 
	...
	private static void batchTest(String ppl, String mzxml) throws FastaDataBaseException, IOException, XMLStreamException, 
		...
		for(int i=0;i<pplfiles.length;i++){
			...
			if(name.endsWith("ppl")){
				String key = name.replace("no-", "with-");
