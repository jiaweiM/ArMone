 ...
 * Analysis and an algorithm", in Advances in Neural Information Processing Systems 14.</li>
 * </ul>
 * </p>
 * <p>
 * This implementation assumes that the data instances are points in an metric space. The algorithm is based on the
 * concept of similarity between points instead of distance. Given two points <var>x</var> and <var>y</var> and their
 * distance <code>d(x, y)</code> (e.g. Euclidean), their similarity is defined as <code>exp(- d(x, y)^2 / (2 *
 * sigma^2))</code>, where <var>sigma</var> is a scaling factor (its default value is 1.0).
 * </p>
 * <p>
 ...
 * their similarity is set to 0. This parameter combined with the use of sparse matrices can improve the performances
 * w.r.t. the memory usage.
 * </p>
 * <p>
 * To classify a new instance w.r.t. the partitions found this implementation applies a naive min-distance algorithm
 * that assigns the instance to the cluster that contains the nearest point. Since the distance to similarity function
 * is bijective and monotone the nearest point is also the most similar one.
 * </p>
 * <p>
 * Valid options are:
 * <ul>
 ...
 * </pre>
 * <p>
 * Specifies the alpha star factor. The algorithm stops the recursive partitioning when it does not find a cut that has
 * a value below this factor.<br> Use this argument to limit the number of clusters.</li>
 * <li>
 *
 * <pre>
 * -S &lt;positive number&gt;
 ...
 * </ul>
 * <p>
 * This implementation relies on the COLT numeric package for Java written by Wolfgang Hoschek. For other information
 * about COLT see <a href="http://acs.lbl.gov/software/colt/">its home page</a>.
 * </p>
...
public class SpectralClusterer extends AbstractClusterer
{
    ...
    /**
     * Computes the association degree between two partitions of a graph.<br> The association degree is defined as the
     * sum of the weights of all the edges between points of the two partitions.
     *
    ...
    /**
     * Returns the best cut of a graph w.r.t. the degree of dissimilarity between points of different partitions and the
     * degree of similarity between points of the same partition.
     *
     * @param W the weight matrix of the graph
    ...
    /**
     * Splits recursively the points of the graph while the value of the best cut found is less of a specified limit
     ...
     */
    @Override
    public void buildClusterer(@SuppressWarnings("hiding") final Instances data)
    {
    ...
    public String[] getOptions()
    {
        ...
        options.add("-D");
        options.add((getDistanceFunction().getClass().getName() + " " + Utils
                .joinOptions(getDistanceFunction().getOptions())).trim());
     ...
     * @throws Exception if alpha_star is not between 0 and 1
     */
     ...
     * @throws Exception if sigma is not strictly greater than 0
     */
     ...
     * @throws Exception if r is not -1 and r is not a positive number
     */
    ...
    /**
     * Returns the capabilities of the algorithm. Attribute types actually supported depend on the selected distance
     * function (i.e., a type is supported only if it is managed by such function).
     */
    @Override
